...
export default function nonUniqueElements(data) {
  ...
  const numberOfEntries = new Map()

  data.forEach(number => {
    if (numberOfEntries.has(number)) {
      numberOfEntries.set(number, numberOfEntries.get(number) + 1)
    }
    else {
      numberOfEntries.set(number, 1)
    }
  })

  return data.filter(number => numberOfEntries.get(number) > 1)
}
